...
def resolve_two_stage_command_classes(class_names: Sequence[str] | None = None) -> tuple[str, ...]:
    if not class_names:
        return tuple(TWO_STAGE_COMMAND_CLASSES)

    normalized: list[str] = []
    seen: set[str] = set()
    for raw in class_names:
        label = normalize_event_label_input(str(raw))
        if not label or label == "RELAX" or label in seen:
            continue
        seen.add(label)
...
def combine_two_stage_public_probabilities(
    ...
    *,
    ...
    if command.shape[-1] <= 0:
        raise ValueError(f"command_probs must have at least 1 class, got shape={command.shape}")
    if command_class_names is not None and command.shape[-1] != len(tuple(command_class_names)):
        raise ValueError(
            "command_probs shape mismatches command_class_names: "
            f"shape={command.shape}, names={tuple(command_class_names)}"
        )
    ...
    class EventOnsetTwoStageDemo3Net(nn.Cell, _EventOnsetFeatureMixin):
        ...
        def __init__(self, config: EventModelConfig):
            ...
            if int(config.num_classes) < 2:
                raise ValueError(
                    "event_onset_two_stage_demo3 requires at least 2 public classes "
                    f"(got num_classes={config.num_classes})"
                )
            self.config = config
            self._init_feature_layers(config)
            self.fusion = nn.SequentialCell(
            ...
            self.command_head = nn.Dense(config.fusion_hidden_dim, int(config.num_classes) - 1)
