...
try:
    ...
    try:
        ...
        _mindspore_lite = importlib.import_module("mindspore_lite")
        LiteContext = getattr(_mindspore_lite, "Context", None)  # type: ignore
        LiteModel = getattr(_mindspore_lite, "Model", None)  # type: ignore
        LiteModelType = getattr(_mindspore_lite, "ModelType", None)  # type: ignore
    except Exception:
...
def _resolve_lite_bindings():
    global LiteContext, LiteModel, LiteModelType
    if LiteContext is not None and LiteModel is not None:
        return LiteContext, LiteModel, LiteModelType
    import importlib

    module = importlib.import_module("mindspore_lite")
    LiteContext = getattr(module, "Context", None)  # type: ignore
    LiteModel = getattr(module, "Model", None)  # type: ignore
    LiteModelType = getattr(module, "ModelType", None)  # type: ignore
    return LiteContext, LiteModel, LiteModelType
...
class EventPredictor:
    ...
    def _load_lite(self, model_path: str | Path) -> None:
        lite_context_cls, lite_model_cls, lite_model_type = _resolve_lite_bindings()
        if lite_model_cls is None or lite_context_cls is None:
        ...
        if hasattr(lite_context, "target") and target:
            try:
                ...
                pass
        self._lite_model = lite_model_cls()
        model_type = 0
        if lite_model_type is not None:
            for attr in ("MINDIR", "MINDIR_LITE", "MINDIR_IR"):
                candidate = getattr(lite_model_type, attr, None)
    ...
    def _build_detail_from_logits(
        ...
        *,
        ...
        if self._is_two_stage_demo3():
            ...
            command_class_names: tuple[str, ...] | None = None
            if self.metadata is not None:
                if self.metadata.command_classes:
                    command_class_names = tuple(self.metadata.command_classes)
                else:
                    metadata_public_names = self.metadata.public_class_names or self.metadata.class_names
                    if len(metadata_public_names) > 1:
                        command_class_names = tuple(str(name) for name in metadata_public_names[1:])
            public_probs = combine_two_stage_public_probabilities(
                gate_probs,
                command_probs,
