...
def _predict_labels_from_output(output, *, model_type: str, class_names: Sequence[str] | None = None) -> np.ndarray:
    if is_two_stage_demo3_model(model_type):
        ...
        probs = combine_two_stage_public_probabilities_from_logits(
            gate_logits,
            command_logits,
            command_class_names=resolve_two_stage_command_classes(class_names),
        )
...
def _call_loss_fn(loss_fn, output, label, *, two_stage: bool):
    if two_stage:
        gate_logits, command_logits = output
        return loss_fn(gate_logits, command_logits, label)
...
if nn is not None:
    ...
    class TwoStageEventLoss(nn.Cell):
        ...
        def construct(self, gate_logits, command_logits, labels):
    ...
    class EventWithLossCell(nn.Cell):
        def __init__(self, backbone, loss_fn, *, two_stage: bool = False):
        ...
        def construct(self, emg, imu, label):
            ...
            return _call_loss_fn(self.loss_fn, logits, label, two_stage=self.two_stage)
    ...
    def _build_loss(self, train_labels: np.ndarray):
        ...
        if is_two_stage_demo3_model(self.model_config.model_type):
            ...
            command_loss_fn = self._build_per_sample_loss(
                ...
                num_classes=len(resolve_two_stage_command_classes(self.class_names)),
            )
        ...
        try:
            for emg, imu, label in dataset.create_tuple_iterator():
                ...
                loss = _call_loss_fn(
                    ...
                    output,
                    label,
                    two_stage=is_two_stage_demo3_model(self.model_config.model_type),
                )
                losses.append(float(loss.asnumpy()))
                pred = _predict_labels_from_output(
                    output,
