...
def collect_file_checks(code_root: Path, wearer_data_root: Path) -> list[Check]:
    ...
    required_files = [
        ...
        code_root / "scripts" / "evaluate_event_demo_control.py",
        code_root / "scripts" / "tune_event_runtime_thresholds.py",
...
def collect_config_checks(code_root: Path) -> list[Check]:
    ...
    checks.append(Check("INFO", "training.recordings_manifest_path", str(data_cfg.recordings_manifest_path)))
    checks.append(Check("INFO", "training.split_manifest_path", str(data_cfg.split_manifest_path)))
    ...
    checks.append(Check("INFO", "runtime.recordings_manifest_path", str(runtime_cfg.data.recordings_manifest_path)))
    checks.append(Check("INFO", "runtime.split_manifest_path", str(runtime_cfg.data.split_manifest_path)))
    if runtime_map and runtime_map.exists():
        try:
            ...
            public_keys = sorted(public_event_label(name) for name in mapping_by_name.keys())
            checks.append(Check("INFO", "runtime.actuation_mapping_keys", ",".join(public_keys)))
    ...
    if runtime_meta and runtime_meta.exists():
        try:
            ...
            expected_class_names = [public_event_label(str(name).strip().upper()) for name in label_spec.class_names]
...
def collect_budget_checks(
    ...
    *,
    ...
    if int(budget_per_class) <= 0:
        checks.append(Check("INFO", "budget.mode", "disabled"))
        return checks
    ...
    for row in rows.values():
        if str(row.get("capture_mode", "")) != data_cfg.capture_mode_filter:
            continue
        target_state = normalize_event_label_input(str(row.get("target_state", "")).strip().upper())
        if not target_state:
            continue
    ...
    target_classes = [public_event_label("CONTINUE"), *[str(key).strip().upper() for key in data_cfg.target_db5_keys]]
    for class_name in target_classes:
        clips = int(clip_counts.get(class_name, 0))
        per_clip = int(data_cfg.idle_top_k_windows_per_clip if class_name == "CONTINUE" else data_cfg.top_k_windows_per_clip)
