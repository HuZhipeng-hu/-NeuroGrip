...
from shared.armband_orientation import normalize_armband_orientation
...
def _resolve_duration_sec(target_state: str, duration_sec: float | None) -> float:
    if duration_sec is not None:
        ...
        return value
    return 4.0 if is_continue_label(target_state) else 3.0
...
def _estimate_activation_quality(
    *,
    ...
    if not is_continue_label(target_state):
...
def _normalize_orientation(value: str) -> str:
    return _sanitize_token(normalize_armband_orientation(value))
...
def _build_default_relative_path(
    *,
    ...
    stem = (
        ...
        f"__ao-{_normalize_orientation(armband_orientation)}"
    )
    ...
    *,
    ...
    if str(output_relpath or "").strip():
        ...
        rel = _build_default_relative_path(
            ...
            armband_orientation=armband_orientation,
            timestamp=timestamp,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--armband_orientation",
        default="unknown",
        help="Explicit wearing direction tag, e.g. normal or flipped.",
    )
    parser.add_argument(
        "--duration_sec",
        type=float,
        default=None,
        help="Total clip length. Default: action=3s, CONTINUE idle clip=4s.",
    )
...
def main() -> None:
    ...
    output_csv, relative_path = _resolve_output_paths(
        ...
        armband_orientation=args.armband_orientation,
        timestamp=timestamp,
    )
    ...
    if is_continue_label(target_state):
    ...
    try:
        while (time.perf_counter() - started) < float(duration_sec):
            ...
            if (not is_continue_label(target_state)) and (not action_cue_sent) and elapsed >= pre_roll_sec:
                print(f"[COLLECT] cue: ACTION NOW -> {target_state}")
                action_cue_sent = True
            if (
                not is_continue_label(target_state)
                and action_cue_sent
    ...
    if not bool(args.no_manifest):
        manifest_row = {
            ...
            "armband_orientation": _normalize_orientation(args.armband_orientation),
