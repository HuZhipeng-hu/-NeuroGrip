...
def test_event_runtime_config_has_model_paths():
    runtime_cfg = load_event_runtime_config(
        Path(__file__).resolve().parent.parent / "configs" / "runtime_event_onset.yaml"
    )
    assert runtime_cfg.model_path.endswith(".mindir")
    assert runtime_cfg.model_metadata_path.endswith(".json")
    assert runtime_cfg.runtime.momentary_action_labels == []
    assert runtime_cfg.hardware.servo_angle_wrist_neutral == 120.0
    assert runtime_cfg.hardware.servo_angle_wrist_cw == 0.0
