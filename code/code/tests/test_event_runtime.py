...
from event_onset.runtime import (
...
def test_event_runtime_command_only_holds_latched_state_and_switches_to_tense_open():
    machine = EventRuntimeStateMachine(
        inference_config=EventInferenceConfig(
            confidence_threshold=0.75,
        ...
        label_to_state={0: GestureType.RELAX, 1: GestureType.TENSE_OPEN, 2: GestureType.THUMB_UP},
    )
    ...
    opened = machine.update(1, 0.93, 8.2, 2_450.0, current_state_confidence=0.60, top2_confidence=0.60)
    assert opened.changed is True
    assert opened.state == GestureType.TENSE_OPEN
    assert opened.emitted_class_name == "TENSE_OPEN"

    hold = machine.update(0, 0.20, 0.2, 4_000.0)
    assert hold.changed is False
    assert machine.current_state == GestureType.TENSE_OPEN
...
def test_event_runtime_rejects_low_energy_false_action_spikes():
    machine = EventRuntimeStateMachine(
        inference_config=EventInferenceConfig(
            confidence_threshold=0.75,
            action_energy_min=3.6,
            per_class_confidence_thresholds={"THUMB_UP": 0.80},
            vote_window=2,
            vote_min_count=2,
            activation_margin_threshold=0.08,
            switch_confidence_boost=0.06,
        ...
        label_to_state={0: GestureType.RELAX, 1: GestureType.THUMB_UP},
    )

    low_energy = machine.update(
        1,
        0.95,
        3.2,
        0.0,
        current_state_confidence=0.10,
        top2_confidence=0.10,
    )
    assert low_energy.changed is False
    assert machine.current_state == GestureType.RELAX
    ...
    assert high_energy.changed is True
    assert high_energy.state == GestureType.THUMB_UP


def test_event_runtime_continue_keeps_wrist_cw_latched_until_explicit_switch():
    machine = EventRuntimeStateMachine(
        inference_config=EventInferenceConfig(
            confidence_threshold=0.75,
            per_class_confidence_thresholds={"TENSE_OPEN": 0.80, "WRIST_CW": 0.65},
            vote_window=2,
            vote_min_count=2,
            activation_margin_threshold=0.08,
            switch_confidence_boost=0.06,
        ...
        low_energy_threshold=2.5,
        class_names=["RELAX", "TENSE_OPEN", "WRIST_CW"],
        label_to_state={0: GestureType.RELAX, 1: GestureType.TENSE_OPEN, 2: GestureType.WRIST_CW},
    )
    machine.update(2, 0.92, 8.0, 0.0, current_state_confidence=0.10, top2_confidence=0.10)
    latched = machine.update(2, 0.93, 8.0, 40.0, current_state_confidence=0.10, top2_confidence=0.10)
    assert latched.changed is True
    assert latched.state == GestureType.WRIST_CW

    hold = machine.update(0, 0.20, 0.2, 1_000.0)
    assert hold.changed is False
    assert machine.current_state == GestureType.WRIST_CW

    machine.update(1, 0.91, 8.1, 1_400.0, current_state_confidence=0.60, top2_confidence=0.60)
    switched = machine.update(1, 0.92, 8.2, 1_440.0, current_state_confidence=0.60, top2_confidence=0.60)
    assert switched.changed is True
    assert switched.state == GestureType.TENSE_OPEN


def test_event_runtime_rejects_momentary_labels_under_command_only():
    try:
        EventRuntimeStateMachine(
            inference_config=EventInferenceConfig(confidence_threshold=0.75),
            runtime_config=EventRuntimeBehaviorConfig(
                release_mode="command_only",
                momentary_action_labels=["WRIST_CW"],
            ),
            low_energy_threshold=2.5,
            class_names=["RELAX", "WRIST_CW"],
            label_to_state={0: GestureType.RELAX, 1: GestureType.WRIST_CW},
        )
    except ValueError as exc:
        assert "momentary_action_labels" in str(exc)
        return
    raise AssertionError("Expected ValueError for momentary_action_labels under command_only")


def test_event_feature_extractor_zeroes_imu_when_runtime_disables_imu():
    data_cfg = EventDataConfig(use_imu=False)
    extractor = EventFeatureExtractor(data_cfg)
    rows = np.zeros((data_cfg.context_samples, 14), dtype=np.float32)
    rows[:, 0] = 5.0
    rows[:, 8:14] = 3.0

    _, imu_feature, energy = extractor.build_inputs(rows)

    assert energy == 0.625
    assert np.allclose(imu_feature, 0.0)
