...
def test_validate_release_contract_requires_tense_open_for_command_only():
    with pytest.raises(ValueError, match="requires class TENSE_OPEN"):
        _validate_release_contract(
            ...
            momentary_action_labels=[],
        )

    with pytest.raises(ValueError, match="requires mapping TENSE_OPEN -> TENSE_OPEN"):
        _validate_release_contract(
            release_mode="command_only",
    ...
    with pytest.raises(ValueError, match="does not allow momentary_action_labels"):
        _validate_release_contract(
            release_mode="command_only",
            class_names=["RELAX", "TENSE_OPEN", "THUMB_UP"],
            mapping_by_name={"RELAX": "RELAX", "TENSE_OPEN": "TENSE_OPEN", "THUMB_UP": "THUMB_UP"},
            momentary_action_labels=["THUMB_UP"],
        )
    ...
    _validate_release_contract(
        ...
        mapping_by_name={"CONTINUE": "CONTINUE", "TENSE_OPEN": "TENSE_OPEN", "THUMB_UP": "THUMB_UP"},
        momentary_action_labels=[],
    )
