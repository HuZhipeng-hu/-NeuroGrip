...
class EventRuntimeStateMachine:
    """Event-onset runtime controller with voting, latching, and optional idle release."""
    ...
    def __init__(
        ...
        *,
        ...
        self.momentary_action_labels = {
            normalize_event_label_input(str(name))
            for name in getattr(runtime_config, "momentary_action_labels", []) or []
            if str(name).strip()
        }
        ...
        if self.release_mode == "command_only" and self.momentary_action_labels:
            raise ValueError(
    ...
    def _accept_action_event(
        ...
        *,
        ...
        if float(energy) < float(self.inference_config.action_energy_min):
            return False
...
def reset_momentary_runtime_state(
    state_machine: EventRuntimeStateMachine,
    decision: RuntimeDecision,
) -> None:
    """Legacy helper for pulse-style debug actions."""
    emitted_label = normalize_event_label_input(decision.emitted_class_name)
    if not decision.changed or emitted_label not in state_machine.momentary_action_labels:
        return
    state_machine.current_label = 0
    state_machine.current_state = state_machine.label_to_state.get(0, GestureType.RELAX)
    state_machine._vote_window.clear()
    state_machine._vote_window.append(0)
    state_machine._idle_since_ms = None
...
class EventFeatureExtractor:
    ...
    def __init__(self, data_config: EventDataConfig):
        ...
        self.use_imu = bool(data_config.use_imu)
    ...
    def build_inputs(self, matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray, float]:
        ...
        if not self.use_imu:
            # Keep the runtime input contract unchanged while disabling motion cues.
            imu_window = np.zeros_like(imu_window, dtype=np.float32)
...
class EventOnsetController:
    ...
    def _process_current(self) -> ControllerStep | None:
        ...
        if decision.changed and self.actuator is not None:
            ...
            if self.state_machine.momentary_action_labels:
                reset_momentary_runtime_state(self.state_machine, decision)
