...
class OrangePiClient(emg_armband.DeviceListener):
    ...
    def _run_inference(self):
        ...
        detail = self.predictor._ckpt_model.predict(spec_input, imu_input) if self.predictor._ckpt_model \
            else self.predictor._lite_model.predict(spec_input, imu_input)
            
        # 解析结果 (假设单阶段模型)
        # detail.public_probs 是 (1, 7) 概率分布
        probs = detail.public_probs[0]
