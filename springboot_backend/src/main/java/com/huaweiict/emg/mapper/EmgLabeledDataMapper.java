...
public interface EmgLabeledDataMapper extends BaseMapper<EmgLabeledData> {
     ...
     * 获取标注统计信息
     */
        @Select("SELECT gesture_label, COUNT(*) AS sample_count, AVG(quality_score) AS avg_quality_score " +
                        "FROM emg_labeled_data WHERE is_valid = TRUE GROUP BY gesture_label")
