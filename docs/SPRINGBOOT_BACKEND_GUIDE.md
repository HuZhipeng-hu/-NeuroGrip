...
EMG 后端是 赛道肌电手势识别系统的**云端核心服务**，基于 **Spring Boot 2.7** 构建，提供 RESTful API 和 WebSocket 双通道实时通信能力，实现边缘设备数据采集、云端存储、AI 训练管理、模型版本控制等完整生命周期管理。
...
📊 数据管理
  ├── 接收 KunpengPro/Windows 上报的 EMG 数据（1000Hz 采样率）
  ├── 实时存储至华为云 RDS MySQL
  ├── 缓存最新数据至内存（低延迟访问）
  └── 数据清理策略（默认保留7天）
...
6) 部署与激活：`/api/model/deploy` 触发部署（cloud/KunpengPro），可选 `setAsActive`；`/api/model/{version}/activate` 切换激活版本。
...
**✅ 数据接收**
- HTTP 批量上报：KunpengPro 一次上传多帧数据（备用通道）
...
| 端点 | 客户端类型 | 用途 | 消息方向 |
|------|-----------|------|----------|
| `/ws/emg` | KunpengPro/Windows | 边缘设备数据上报 | 设备 → 服务器 |
...
**端点**: `GET /api/emg/history?deviceId=KunpengPro_01&limit=100`
...
**端点**: `GET /api/annotation/cache-data?deviceId=KunpengPro_01&startTime=2024-03-08T12:00:00&endTime=2024-03-08T12:01:00`
...
mybatis-plus:
  ...
  type-aliases-package: com..emg.entity
  configuration:
